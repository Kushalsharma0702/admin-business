...
export function initSchema() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS users (
      ...
      occupation      TEXT,
      client_since    TEXT,
      portal_status   TEXT DEFAULT 'active',  -- 'active' | 'pending' | 'none'
      created_at      TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
      updated_at      TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now'))
    );

    -- ─── Tasks (portal tasks assigned to clients by admin) ────────────────────
    ...
    CREATE TABLE IF NOT EXISTS documents (
      ...
      file_size         INTEGER DEFAULT 0,
      section_name      TEXT,
      document_type     TEXT DEFAULT 'ocr',
      storage_path      TEXT NOT NULL,
      status            TEXT DEFAULT 'uploaded',
    ...
    CREATE TABLE IF NOT EXISTS employees (
      ...
      department            TEXT,
      hourly_rate           REAL,
      federal_tax_credit    REAL,
      provincial_tax_credit REAL,
      salary                REAL,
      metadata              TEXT DEFAULT '{}',
      created_at            TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
      updated_at            TEXT DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    ...
    CREATE INDEX IF NOT EXISTS idx_payroll_entries_client_id ON payroll_entries(client_id);
