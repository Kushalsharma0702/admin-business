...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-WSZ5ixgD.mjs");
...
const manifest = {
  "a5df2aa3bf7578996b06e3864a107af9c34ab84116685c3abc81ac3f4447536e": {
    functionName: "extractInvoiceText_createServerFn_handler",
    importer: () => import("./ocr.functions-D7ChwigF.mjs")
  }
};
...
async function executeMiddleware$1(middlewares, env2, opts) {
  ...
  const callNextMiddleware = async (ctx) => {
    ...
    try {
      if ("inputValidator" in nextMiddleware.options && nextMiddleware.options.inputValidator && env2 === "server") ctx.data = await execValidator(nextMiddleware.options.inputValidator, ctx.data);
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./router-CxdnWRBk.mjs").then((n2) => n2.a),
